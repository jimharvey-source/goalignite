...
module.exports = async (req, res) => {
  ...
  const doc = new PDFDocument({
    ...
    bufferPages: true,
    info: { Title: `${tool.name} — ${(body.result && body.result.taskTitle) || "report"}`, Author: "Jim Harvey | The Message Business" },
  });

  const safeTitle = ((body.result && body.result.taskTitle) || tool.name).replace(/[^\w \-]/g, "").slice(0, 60).trim() || tool.name;
...
module.exports._render = { renderDelegate, renderGoal, stamp, TOOLS, makeHelpers, drawCadenceCard, drawInfoCard };
